...
problemConfiguration.bSplineOrder_time = 6;
problemConfiguration.bSplineOrder_space = 6;
...
% Example 4
% problemConfiguration.f_time = {@(t) 0};
% problemConfiguration.f_space = {@(x) 0};
% problemConfiguration.u_0_x = @(x)  x .* (x < 0.5) + (1-x) .* (x>= 0.5);
% problemConfiguration.u_1_x = @(x) 0*x;
% 
% problemConfiguration.dAlemebert = true;
...
methods = [4 5]; % Specify what to test:
...
%                         tCoarse = 0.5 * (tCoarse(1:end-1) + tCoarse(2:end));
                        [solBackslash] = get1Dsolution(problem, U_backslash, res,true);
%                         
...
%                         solBackslash = F(Xfine, Tfine);
                        ...
                        solBackslash = get1Dsolution(problem, U_backslash, resolution ,false);
                    end
...
triangle_y = [minError, 1/sqrt(2) * minError, 1/sqrt(2) * minError,minError];
semilogy(triangle_x, triangle_y, 'k', 'LineWidth', 2)
text(refinements(end)-1.25, 0.9 * minError, '1')
text(refinements(end)-0.5, 0.95 * 1/sqrt(2) * minError, '1')
legend(labels)
title('Error decay')
xlabel('Refinements')
ylabel('L_2 error')
