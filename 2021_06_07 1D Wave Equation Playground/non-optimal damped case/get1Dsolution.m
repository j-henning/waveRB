...
function [sol, x, t] = get1Dsolution(p, U,  resolution)
mu = 1;
if isfield(p, 'mu')
    mu = p.mu;
end
...
for i=1:size(u_full,1) % every space node in x
    ...
    space_x_indices = 1:length(spline_x);%find(spline_x, 1, 'first'):find(spline_x, 1, 'last');
    ...
    for k=1:size(u_full,2) % every time node
        ...
        time_indices = 1:length(spline_t);%find(spline_t, 1, 'first'):find(spline_t, 1, 'last');
