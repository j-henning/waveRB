...
function [p] = create3DWaveProblemDamped(pC)
...
p.A_time = pC.eps * StiffMat(p.T_time_ansatz, ... % Tsol
    p.T_time_test, ... % Ttest
    p.bSplineOrder_time_ansatz, ... %ksol
    p.bSplineOrder_time_test, ... %ktest
    p.offset_time_ansatz,... %offsetsol
...
if ~isempty(p.f_time)
    ...
    for i=1+p.offset_time_test(1):length(p.T_time_test) - p.bSplineOrder_time_test - p.offset_time_test(2)
        ...
        v0 = Ndiff(p.T_time_test, p.bSplineOrder_time_test,1,i,0);
        
        if v_dot0 == 0 && v0 == 0
            continue;
        end
        
        p.rhs(:,i) = p.rhs(:,i) + reshape((-v_dot0+pC.eps*v0) * U0, ...
            [numel(U0) 1]);
    end
