...
for refinementLevel_space = space_refinement
    for refinementLevel_time = refinementLevel_space
        ...
        problem = create3DWaveProblemDamped(problemConfiguration);
        fprintf(' Done!\n')
...
%                       B = kron(problem.D_time, problem.M_space) ...
%                         - kron(problem.A_time, problem.M_space) ...
%                         + kron(problem.M_time, problem.A_space);
                    
                    funB = @(x) reshape(problem.M_space * reshape(x, [dimS dimT]) * problem.D_time' ...
                        - problem.M_space * reshape(x, [dimS dimT]) * problem.A_time'  ...
                    + problem.A_space * reshape(x, [dimS dimT]) * problem.M_time', [dimS * dimT, 1]);
...
%                     norm(funB(problem.rhs(:)) - B * problem.rhs(:))
                    ...
                    tt = tic;
                     U_backslash = gmres(funB, problem.rhs(:),200,1e-10);
