...
function [sol] = get1DsolutionDifferentSplines(p, U,  resolution, time_refinement)
mu = 1;
if isfield(p, 'mu')
    mu = p.mu;
end
...
u = reshape(full(U), p.dim_space, numel(U) / p.dim_space);
...
% else
    ...
    splines_time = zeros(2^time_refinement, length(t));
    splines_2_time = zeros(2^time_refinement, length(t));
    
    for i=1:size(splines_time,1)
        %         if i < 5 || i > size(splines_time,1) - 5
        
        g = timeSplines(time_refinement,i,0);
        h = timeSplines(time_refinement,i,2);
        
        splines_time(i,:) = g(t);
        splines_2_time(i,:) = h(t);
