...
z = linspace(0,1, 2^level+1);
K = 2^ level+1; % Number of time steps




for refinement_space = level%space_refinements
    for refinement_time = level%refinement_space%time_refinements
        fprintf('#######################################\n')
        ...
        tau = 1 / (K-1);
        sol_ana = zeros(length(x), length(y), length(z), K);
            ...
            for j=1:size(u_full,2) % every space node in y
                ...
                for k=1:size(u_full,3) % every space node in z
                    ...
                    for l=1:K % Every time step
                        ...
                        sol_ana(space_x_indices, space_y_indices, space_z_indices, l) = ...
                            sol_ana(space_x_indices, space_y_indices, space_z_indices, l) + ...
...
save('data/3D-example3.mat', 'sol_ana', '-v7.3')
