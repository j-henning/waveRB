...
function [p] = create1DWaveProblemDifferentSplines(pC)
mu = 1;
if isfield(pC, 'mu')
    mu = pC.mu;
end
...
p.Q_time = zeros(2^p.refinementLevel_time, 2^p.refinementLevel_time); % 2,2

for i=1:2^p.refinementLevel_time
    for j=1:2^p.refinementLevel_time
        f1 = timeSplines(p.refinementLevel_time,i,2);
        f2 = timeSplines(p.refinementLevel_time,j,2);
        f = @(x) f1(x) .* f2(x);
        p.Q_time(i,j) = integral(f,0,1, 'RelTol', 1e-10);
    end
end

p.M_time = zeros(2^p.refinementLevel_time, 2^p.refinementLevel_time); % 0,0

for i=1:2^p.refinementLevel_time
    for j=1:2^p.refinementLevel_time
        f1 = timeSplines(p.refinementLevel_time,i,0);
        f2 = timeSplines(p.refinementLevel_time,j,0);
        f = @(x) f1(x) .* f2(x);
        p.M_time(i,j) = integral(f,0,1, 'RelTol', 1e-10);
    end
end

p.D_time = zeros(2^p.refinementLevel_time, 2^p.refinementLevel_time); % 2,0
...
for i=1:2^p.refinementLevel_time
    for j=1:2^p.refinementLevel_time
        ...
        f2 = timeSplines(p.refinementLevel_time,j,0);
        f = @(x) f1(x) .* f2(x);
        p.D_time(i,j) = integral(f,0,1, 'RelTol', 1e-10);
    end
end
...
if ~isempty(p.f_time)
    warning('Only using u0, f and u1 are currently not implemented!')
...
%         end
        ...
        % Get the first time spline
        for i=1:1 % All time splines which do not vanish for t = 0
        spline_time = timeSplines(p.refinementLevel_time,i,0);
        spline_time = spline_time(0);
        spline_time_t = timeSplines(p.refinementLevel_time,i,1);
        spline_time_t = spline_time_t(0);
...
%                 rhs_x_2(j-p.offset_space_test(1)) = rhs_x_2(j-p.offset_space_test(1)) + int;
        ...
        p.rhs(:,i) =  rhs_x_1 + rhs_x_2;
        end
        
        % increase the size of p.rhs 
        
        p.rhs(end,2^p.refinementLevel_time) = 0; % TODO: Remove this when using a proper rhs!!!
...
%      p.nsol_time = length(p.T_time) - p.bSplineOrder_time;
     p.nsol_time = 2^p.refinementLevel_time+2;
     p.nsol_space = length(p.T_space) - p.bSplineOrder_space;
     p.dim_space = p.nsol_space - (p.offset_space_ansatz(1)+p.offset_space_ansatz(2));
