% Solve A1 ((L2 L2')\X)  +  X B = C1 C2' by the Rational Krylov subspace
% method (Galerkin condition onto the Rational Krylov subspace)
% Input:
% A1     - Coefficient matrix
% A2     - Coefficient matrix
% L2     - Lower diagonal matrix
% B      - Coefficient matrix
% C1     - Right hand side factor
% C2     - Right hand side factor
% m      - Maximum space dimension
% tol    - Stopping tolreance (backward error)
% s1     - Estimates for real spectral interval
% smax   - Estimates for real spectral interval
% ch     - ch=1 for complex poles and ch=0 real poles
% period - How often the projected problem is solved
% Output:
% Z1     - Factors such the solution is given through Z1 * Z2'
% Z2     - Factors such the solution is given through Z1 * Z2'
% nrmres - Final residual norm
% sdim   - Maximum space dimension
% Hints:
...
function [Z1,Z2,nrmres,sdim]=arn_adapt_SylvMRHS_oneside_lyapprec(A1,A2,L2,B,C1,C2,m,tol,s1,smax,ch,period)
...
while i < m
    ...
    while (paired==0),
        ...
        if (cmplxflag), snew=conj(snew); s=[s,snew];cmplxflag=0; %newAv=EL\(A*(EU\V(:,i1)));
            ...
            g = VV'*newAv; g1 = g; g2 = (((V'/L2)*A1)/L2')*VV; g3 = V'*newAv; %(A1*(A2\V));
    ...
    if (rem(i,period)==0 | i >= m | max(js,js) >= n)
        ...
        [qq,rr]=qr([VV(:,1:js),L2\(A1*(L2'\VV(:,1:js)))],0);
