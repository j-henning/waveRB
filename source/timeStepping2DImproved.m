...
f_space_x = {@(x) 0*x};
f_space_y = {@(y) 0*y};
u0= @(x,y) .1 * ((x-0.5).^2 + (y-0.5).^2 <= 0.2^2);
% u0= @(x,y) 1 + 0.* x .*y;
u1 = @(x,y)  0.*x.*y;
% u_analytical = @(x,y,t) 0.*t.*x.*y; % Not known
inital_conditions = true;
...
for i=1:2^8+1
    for j=1:2^8+1
        ...
        sol_ref(i,j,:) = dAlemenbertSphere(r,t,c, u0r);
    end
end
...
for refinement_space = 1:5%space_refinements
    for refinement_time = refinement_space%time_refinements
        ...
        if laplace
            ...
            funA = @(x) c^2 * reshape(M1D * reshape(x, [dimM dimM]) * S' ...
...
%                 + kron(M1D, S) + kron (Q, O);
            ...
            funA = @(x)  c^2* reshape(M1D * reshape(x, [dimM dimM]) * Q' ...
...
%         end
        ...
        U0 = zeros(ntest-sum(test_offset),ntest-sum(test_offset));
         ...
         if inital_conditions
            for j=1+test_offset(1):ntest-test_offset(2)
                for k=1+test_offset(1):ntest-test_offset(2)
                    g = @(y)@(x) u0(x,y) .* Ndiff(T_space_test, splineOrder,0,j,x) .* Ndiff(T_space_test, splineOrder,0,k,y); %*u0
                    ...
                    for stepX = 0:splineOrder-1
                        for stepY = 0:splineOrder-1
                            ...
                            a = [T_space_test(j+stepX); T_space_test(k+stepY)];
                            b = [T_space_test(j+stepX+1); T_space_test(k+stepY+1)];
                            U0(j-test_offset(1), k-test_offset(1)) = U0(j-test_offset(1), k-test_offset(1)) ...
                                + Gaussq(a,b,g,5);
...
%         end
        ...
        l2error(refinement_space, refinement_time) = sqrt(mean((solInt-sol_ref).^2, 'all','omitnan'));
    end
end

figure
diag(l2error)
semilogy(diag(l2error), '*--'), grid on
drawnow
...
for i=1:size(solInt,3)
    subplot(1,2,1)
   s = surf(squeeze(sol(:,:,i)));s.EdgeColor = 'none';
   
    subplot(1,2,2)
   s = surf(squeeze(sol_ref(:,:,i)));s.EdgeColor = 'none';
