...
%                         calculate1DSolvingError(problem, U_cg_lyap);
            ...
            switch ii
                ...
                case 4
                    ...
                    fprintf('Testing the backslash solution\n');
                     solMldivide = get1Dsolution(problem, U, res);
            end
            
            %% Smoothing (just in time)
            [X, T] = meshgrid(linspace(0,1,2^res.x+1), linspace(0,1,2^res.t+1));
            [Xfine, Tfine] = meshgrid(x,t);
            solMldivide = movmean(solMldivide,2);
            solMldivide = interp2(X,T,solMldivide,Xfine,Tfine);
            ...
            if l2error
                switch ii
                    ...
                    case 4
                        errorBackslash(refinementLevel_space, refinementLevel_time)  = sqrt(mean( (solMldivide-sol_ana).^2, 'all'))
                end
            end
...
plot(x,solMldivide(:,1), 'g:', 'LineWidth', 2)
...
plot(x,solMldivide(:,end), 'g:', 'LineWidth', 2)
...
% open(v)
for i=1:5:size(sol_ana,2)
    plot(x,sol_ana(:,i), 'LineWidth', 2), hold on
%     plot(x,solLyap(:,i), 'r--', 'LineWidth', 2)
    plot(x,solMldivide(:,i), 'g:', 'LineWidth', 2)

     plot(x,solInt(:,i),10, 'm--', 'LineWidth', 2)
    xlabel('x')
    ylabel('u(x,0)')
    legend('Reference solution', 'Space Time (Mldivide)', 'Timestepping')
